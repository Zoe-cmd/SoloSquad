...
8. **变更单一入口**：所有需求变更必须通过项目经理，不得直接告诉执行角色
9. **变更分级**：L1（简单变更）由项目经理决定，L2（复杂变更）需技术负责人评估，L3（紧急变更）先执行后补审
10. **变更文档化**：所有变更必须记录在 `docs/变更总览.md` 和 `docs/变更记录/` 目录
11. **变更通知**：相关角色必须读取变更文档后才能执行变更
...
3. **读取该角色需要的共享规范**：读取 `{SKILL_ROOT}/shared/` 目录下该角色「必需文档」章节列出的文件，不要读取所有共享规范
...
3.5 读取 docs/工作日志/工作日志-{角色代号}.md（若存在）→ 恢复进度，声明角色身份【锚定#0】
4. 分析需求
5. 设计方案
...
├── shared/                      ← 共享规范（按角色需要加载，见角色文件的「必需文档」章节）
├── templates/                   ← 12 个文档模板
